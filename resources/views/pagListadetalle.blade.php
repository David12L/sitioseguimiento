...
@section('seccion')
    ...
    <form action="{{ route('Estudiante.xRegistrar') }}" method="post" class="d-grid gap-2">
        ...
        @error('praMod1')
            <div class="alert alert-danger">
                Información de la práctica modular 1 es necesario
            </div>
        @enderror

        @error('praMod2')
            <div class="alert alert-danger">
                Información de la práctica modular 2 es necesario
            </div>
        @enderror

        @if($errors ->has('praMod3'))
            <div class="alert alert-warning alert-dismissible fade show" role="alert">
                Información <strong>practica modular 3 </strong> es requerido
    ...
    <br>
    <h3>Lista. Estamos en pág. {{ $xAlumnosDetalle->currentPage() }} de {{ $xAlumnosDetalle->count() }} </h3>
